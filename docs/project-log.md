...
- M4已完成用户批准的A档：39条新样本（18validation/21test）、33条开发标签overlay，51条查询三模式真实检索，6条validation真实Agent。实际调用65次，上限81；动作匹配5/6，3条草稿虚称已转交。72条标签和6条原始输出由用户委托Agent审查，不是独立人工标注。全量152 passed（108逻辑/替身＋44真实PG）；剩余33条Agent样本未运行。详见[M4报告](m4-evaluation.md)、[审阅清单](m4-label-review.md)、[调用记录](m4-call-budget.md)。按用户最新要求仅提交M4，M5不推进；上次中断前M5补丁未落盘，无工作台文件或依赖改动。
...
- M3已实现三模式、原生BM25与RRF；M4完成A档条件下的质量评测，独立test的Agent评测尚未执行；M5工作台未实现。
