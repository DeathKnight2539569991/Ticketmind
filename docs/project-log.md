...
> 更新：2026-09-15。仅维护当前能力、关键决策、验证证据与待办。
> 本轮 M3 从 main / a827fa9 开始，初始工作区干净；下方 M2 未提交描述属于历史记录。M3 实现与验收完成，提交和远端状态以 Git 记录为准；未部署，保留原数据与缓存。
...
- M3 已实现三模式、原生 BM25 与 RRF，接入首次/再次检索并完成真实检索及 HTTP/数据库验证。M4 独立评测、M5 工作台尚未完成。
