"""Real HTTP + isolated PostgreSQL + exact query cache + live Milvus + optional one decision."""
import argparse
import json
import secrets
import socket
...
def main():
    ...
    query = build_retrieval_query(subject=initial["subject"], messages=initial["messages"])
    embedding = CachedQueryEmbeddings(
        qwen,
        root / "data/cache/graph/api_timeout/query.json",
    ...
    try:
                ...
                try:
                    ...
                    with httpx.Client(base_url=f"http://127.0.0.1:{port}", trust_env=False, timeout=110,
                        ...
                        created = client.post("/tickets", headers={"Idempotency-Key": uuid4().hex},
                                              json={"subject": initial["subject"], "body": initial["messages"][0].content,
                                                    "channel": "api", "requester_role": "synthetic-user"})
                        assert created.status_code == 201, created.text
                        ticket_id = created.json()["id"]
                        detail = client.get(f"/tickets/{ticket_id}").json()
                        assert detail["subject"] == initial["subject"] and detail["messages"][0]["body"] == initial["messages"][0].content
                        key = uuid4().hex
                        ...
                        with factory() as session:
                            ...
                            assert "body" not in saved.input_snapshot and saved.input_snapshot["messages"][0]["body"] == initial["messages"][0].content
                        report = {"verification": "real_http_postgresql_and_milvus_with_m0_model_cache",
                                  "isolated_schema": schema, "ticket": {"subject": initial["subject"], "messages": [m.model_dump() for m in initial["messages"]]}, "result": result,
        ...
        counts = {"embedding_calls": embedding.calls, "embedding_cache_hits": embedding.cache_hits,
