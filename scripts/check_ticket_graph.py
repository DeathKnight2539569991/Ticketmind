...
def run_check(*, cache_dir: Path, check_only: bool = False, allow_embedding: bool = False) -> None:
    ...
    embeddings = CachedQueryEmbeddings(
        settings,
        cache_dir / "query.json",
        allow_call=allow_embedding,
    ...
    try:
        ...
        print(
            json.dumps(
                {
                    "embedding_attempts": embeddings.calls,
                    "query_cache_hits": embeddings.cache_hits,
                }
            )
        )
...
def validate_result(initial_state: TicketAgentState, final_state: TicketAgentState) -> None:
    ...
    assert final_state["retrieval_query"] == build_retrieval_query(
        subject=initial_state["subject"],
        messages=initial_state["messages"],
    )
...
def main() -> None:
    parser = argparse.ArgumentParser(description="验证结构化工单输入 → 检索")
