...
from ticketmind.agent.decide import decide_ticket
...
class AgentRunner:
    ...
    def __init__(self, qwen: QwenSettings, milvus: MilvusSettings, config: ProcessingSettings,
                 *, understanding_fn=None, embedding_factory=None, decision_fn=None):
    ...
    def __call__(self, snapshot: dict) -> RunOutput:
        started = monotonic()
        partial = {"subject": snapshot["subject"], "body": snapshot["body"]}
        evidence, usage = [], {}
        ...
        try:
            ...
            client = build_milvus_client(self.milvus.model_copy(update={"timeout_seconds": min(self.milvus.timeout_seconds, remaining())}))
            ...
            embeddings = self.embedding_factory(remaining) if self.embedding_factory else build_budgeted_embeddings(
                self.qwen, remaining, lambda value: usage.update(embedding=value))
