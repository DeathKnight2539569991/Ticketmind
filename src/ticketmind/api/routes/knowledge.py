...
def sync_case(request, dataset, source_id):
    factory = request.app.state.session_factory
    with factory() as session:
        before = require_case(session, dataset, source_id)
        observed_version = before.version
        if before.status == "active" or (before.status == "retired" and before.index_error is None):
            return read_case(before)
    if request.app.state.knowledge_sync is not None:
        return request.app.state.knowledge_sync.one(dataset, source_id)
    client = None
    try:
        ...
        return KnowledgeSync(factory, MilvusKnowledgeIndex(client, milvus.timeout_seconds), qwen).one(dataset, source_id)
...
def approve(ticket_id: UUID, payload: KnowledgeWrite, actor: ReviewerDependency, key: IdempotencyKey,
