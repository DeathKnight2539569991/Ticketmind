...
def _create_run(session_factory, runner_factory, ticket_id, payload, actor_id, key, *, workflow):
    ...
    try:
        ...
        with session_factory() as session, session.begin():
            ...
            # Exception chains can contain model output, validation input values,
            # and provider response bodies. Keep routine logs structural only.
            logger.error("run_id=%s stage=%s code=%s error=%s",
                         run_id, stage, run.error_code, type(cause or exc).__name__)
