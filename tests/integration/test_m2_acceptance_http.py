...
def ledger(path, **limits):
    return dev_acceptance.AttemptLedger(path / "attempts.json",
                                       {c: limits.get(c, 0) for c in dev_acceptance.M4_CATEGORIES})
...
def test_acceptance_exports_raw_and_reviewed_results(tmp_path, monkeypatch, case_index, review_decision, status):
    ...
    def generate(**kwargs):
        content = (proposal.model_dump_json(exclude={"risk_flags"}) if kwargs["settings"].model == config.decision_model
                   else '{"violations": []}')
        assert kwargs["settings"].model in {config.decision_model, config.judge_model}
    ...
    budget = ledger(tmp_path, initial_embedding=1, decision=1, judge=1)
